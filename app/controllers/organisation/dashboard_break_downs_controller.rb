class Organisation::DashboardBreakDownsController < Organisation::BaseController
  ...
  def critical_important_system
    ...
    if @type.blank?
      ...
      @suppliers = Supplier.where(unit_id: managing_nodes).joins(:supplier_steps).where(supplier_steps: { importance_level: params[:supplier_type]}).group_by{ |e| e.consumption_model } if params[:supplier_type].present?
      if params[:regions].present?
        regional_ids = Unit.where(id: managing_nodes).where(type:"Units::Regional").where(name: @regional_filters).ids
        @suppliers = Supplier.where(unit_id: regional_ids || managing_nodes).joins(:supplier_steps).where(supplier_steps: { importance_level: params[:supplier_type]}).group_by{ |e| e.consumption_model } if params[:regions].present?
      end
    ...
    @private_suppliers = Supplier.where(unit_id: managing_nodes).joins(:supplier_steps).where(supplier_steps: { importance_level: params[:supplier_type], party_type: 'firm-hosted'}) if params[:supplier_type].present?
